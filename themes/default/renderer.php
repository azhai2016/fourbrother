...
class renderer
{
    public function menu_icon($menu)
    {
        global $SysPrefs;
    ...
    public function wa_header()
    {
        page(_($help_context = ''), false, true);
    }

    public function wa_footer()
    {
        end_page(false, true);
    }
    ...
    public function menu_header($title, $no_menu, $is_index)
    {
        ...
        if (!$no_menu) {
            ...
            if ($SysPrefs->help_base_url != null) {
                echo "<a target = '_blank' onclick=" . '"' . "javascript:openWindow(this.href,this.target); return false;" . '" ' . "href='" . help_url() . "'>" . $himg . '<span>' . _('帮助') . "</span></a>";
            }

            echo "<a class='shortcut' href='" . $path_to_root . "/access/logout.php?'>" . $img . '<span>' . _('注销') . "</span></a>";
