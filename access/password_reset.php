...
$title = $SysPrefs->app_title . ' ' . $version . ' - ' . _('重置密码');
...
echo "<a target='_blank' href='" . $SysPrefs->power_url . "'><img src='" . $path_to_root . '/themes/' . $def_theme . "/images/fourborther_erp.png' alt='CBSMS' height='50' onload='fixPNG(this)' border='0' ></a>";
echo "</td>\n";
end_row();

echo "<input type='hidden' id=ui_mode name='ui_mode' value='" . fallback_mode() . "' >\n";
table_section_title(_('版本：') . ' ' . $version . "   Build " . $SysPrefs->build_version . ' - ' . _('重置密码'));
echo "<tr><td colspan='2'></td></tr>";
echo "<tr><td class='login_input'><div class='input_container'><i class='fas fa-envelope' title='" . _('邮件') . "'></i>";
...
echo "<tr><td class='login_input'><div class='input_container'><i class='fas fa-building' title='" . _('公司') . "'></i>";
if (!@$SysPrefs->text_company_selection) {
    ...
    echo "<input required type='text' name='company_login_nickname' placeholder='" . _('公司') . "'>";
}

echo '</div></td></tr>';

start_row();
label_cell('请输入你的邮箱地址', "colspan=2 align='center' id='log_msg'");
end_row();

start_row();
echo "<td colspan='2'><center><input type='submit' value='&nbsp;&nbsp;" . _('发送密码') . "&nbsp;&nbsp;&#8250;' name='SubmitReset' onclick='set_fullmode();'></center></td>\n";
end_row();
end_table(1);
