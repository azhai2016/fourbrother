...
if (get_post('addupdate')) {
    ...
    if ($_POST['description'] == '') {
        $input_error = 1;
        display_error(_('角色说明不能为空'));
        set_focus('description');
    } elseif ($_POST['name'] == '') {
        $input_error = 1;
        display_error(_('角色名称不能为空'));
        set_focus('name');
    }
    ...
    if (get_post('role') == $_SESSION['wa_current_user']->access) {
        if (!isset($_POST['Area' . $security_areas['SA_SECROLES'][0]])
            ...
            display_error(_('必须为您的帐户启用“公司设置”部分中的访问级别版本。'));
            $input_error = 1;
            set_focus(!isset($_POST['Section' . SS_SETUP]) ? 'Section' . SS_SETUP : 'Area' . $security_areas['SA_SECROLES'][0]);
        }
    }
    ...
    if ($input_error == 0) {
        ...
        if ($new_role) {
            ...
            display_notification(_('添加了新的安全角色。'));
        } else {
            ...
            display_notification(_('安全角色已更新。'));
        }
        $new_role = true;
        clear_data();
...
if (get_post('delete')) {
    if (check_role_used(get_post('role'))) {
        display_error(_('此角色当前已分配给某些用户，无法删除'));
    } else {
        delete_security_role(get_post('role'));
        display_notification(_('安全角色已成功删除。'));
        unset($_POST['role']);
    }
    $Ajax->activate('_page_body');
}
...
start_row();
security_roles_list_cells(_('角色:') . '&nbsp;', 'role', null, true, true, check_value('show_inactive'));
$new_role = get_post('role') == '';
check_cells(_('显示不活跃:'), 'show_inactive', null, true);
end_row();
end_table();
echo "<hr>";
...
text_row(_('角色名称:'), 'name', null, 20, 22);
text_row(_('角色描述:'), 'description', null, 50, 52);
record_status_list_row(_('状态:'), 'inactive');
end_table(1);
...
foreach (sort_areas($security_areas) as $area => $parms) {
    ...
    if ($newsec != $sec || (($newext != $ext) && ($newsec > 99))) { // features set selection
        ...
        $sec = $newsec;
        $m = $parms[0] & ~0xff;
        label_row($security_sections[$m] . ':', checkbox(null, 'Section' . $m, null, true, _('功能的开/关设置')), "class='tableheader2'", "class='tableheader'");
    }
...
if ($new_role) {
    submit_center_first('Update', _('更新'), '', null);
    submit_center_last('addupdate', _('写入新角色'), '', 'default');
} else {
    submit_center_first('addupdate', _('保存角色'), '', 'default');
    submit('Update', _('更新'), true, '', null);
    submit('clone', _('复制角色'), true, '', true);
    submit('delete', _('删除角色'), true, '', true);
    submit_center_last('cancel', _('取消'), _('取消保存'), 'cancel');
}
