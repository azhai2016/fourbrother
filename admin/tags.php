    ...
    if (can_process()) {
        if ($selected_id != -1) {
            if ($ret = update_tag($selected_id, $_POST['name'], $_POST['description'])) {
                display_notification(_('已更新选定的标记设置'));
            }

        } else {
            if ($ret = add_tag($_POST['type'], $_POST['name'], $_POST['description'])) {
                display_notification(_('添加了新标签'));
            }
...
function can_delete($selected_id)
{
    ...
    if (db_num_rows($result) > 0) {
        display_error(_('无法删除此标记，因为已创建引用它的记录。'));
        return false;
    }
...
if ($Mode == 'Delete') {
    if (can_delete($selected_id)) {
        ...
        display_notification(_('所选标签已被删除'));
    }
    $Mode = 'RESET';
}
...
text_row_ex(_('标签名:'), 'name', 15, 30);
text_row_ex(_('标签说明:'), 'description', 40, 60);
hidden('type');
