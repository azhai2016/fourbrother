 ...
 ***********************************************************************/
$page_security = 'SA_SYSTARGENT';
$path_to_root = '..';
include $path_to_root . '/includes/session.php';

page(_($help_context = '目标参数设置'));

include $path_to_root . '/admin/db/targent_db.inc';
	...
	if ($input_error !=1) {
		if ($selected_id != -1) {
			update_item_category($selected_id, $_POST['description'],
				...
				0, 0, 
				0, 0,
				$_POST['units'], $_POST['mb_flag'],0,	0,
				check_value('no_sale'), check_value('no_purchase'));
			display_notification(_('选择的商品分类已更新'));
		} 
		else {
			add_item_category($_POST['description'],
				$_POST['tax_type_id'],
				$_POST['sales_account'], 
				'0',//$_POST['cogs_account'], 
				'0',//$_POST['inventory_account'], 
				'0',//$_POST['adjustment_account'], 
				'0',//$_POST['wip_account'], 
				$_POST['units'], $_POST['mb_flag'],	
				'0',//$_POST['dim1'],	
				'0',//$_POST['dim2'],	
				check_value('no_sale'), check_value('no_purchase'));
			display_notification(_('新商品分类已添加'));
		}
		$Mode = 'RESET';
	}
}
...
if ($Mode == 'Delete') {
	...
	if (key_in_foreign_table($selected_id, 'stock_master', 'category_id'))
		...
		display_notification(_('选择的商品分类已经删除'));
	}
	$Mode = 'RESET';
}
...
if ($fixed_asset)
	$th = array(_('分类名称'), _('税类型'), _('单位'), _('销售方式'), _('资产账户'), _('折旧成本账户'), _('折旧/处置账户'), '', '');
else
	$th = array(_('分类名称'), _('税类型'), _('单位'), _('类型'), _('销售方式'), _('存货账户'), _('调整账户'), _('销售部门'), '', '');
...
while ($myrow = db_fetch($result)) {
	...
	edit_button_cell('Edit'.$myrow['category_id'], _('编辑'));
	delete_button_cell('Delete'.$myrow['category_id'], _('删除'));
	end_row();
}
...
text_row(_('分类名称:'), 'description', null, 30, 30);  

table_section_title(_('新商品的默认值'));

item_tax_types_list_row(_('商品税率:'), 'tax_type_id', null);

if (is_fixed_asset(get_post('mb_flag')))
	hidden('mb_flag', 'F');
else
	stock_item_types_list_row(_('商品属性:'), 'mb_flag', null, true);

stock_units_list_row(_('计量单位:'), 'units', null);

if (is_fixed_asset($_POST['mb_flag'])) 
	hidden('no_sale', 0);
else
	check_row(_('禁止销售:'), 'no_sale');

check_row(_('禁止采购:'), 'no_purchase');

gl_all_accounts_list_row(_('销售账户:'), 'sales_account', $_POST['sales_account']);
